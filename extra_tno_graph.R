...
#tno_filter_queue_20 <- filter(tno_sim_20_16_5, referral < target_20_date_5$dates)
...
# #### Adjusted plot
tno_20_adjusted_queue_5 <- tno_queue_20_16_5

tno_20_adjusted_queue_2_5 <-
  tno_queue_20_16_5 %>% 
  filter(meet_future_20 == 1)

tno_20_adjusted_queue_2_5$queue_size <-rpois(nrow(tno_20_adjusted_queue_2_5), target_queue_size_20_5)

tno_20_adjusted_queue_5<-
  tno_20_adjusted_queue_5 %>% 
  left_join(tno_20_adjusted_queue_2_5, by = "dates", keep = TRUE) %>% 
...
Extra_tno_graph <-
  ...
  geom_line(col=colours[2], data=tno_20_adjusted_queue) +
  geom_ribbon(data= data.frame(dates= tno_40_adjusted_queue$dates
                               , ymin = tno_40_adjusted_queue$queue_size
                               , ymax = tno_20_adjusted_queue$queue_size)
  ...
  geom_ribbon(data= data.frame(dates= tno_queue16_5$dates
                               , ymin = tno_queue16_5$queue_size
                               , ymax = tno_queue16_2$queue_size)
...
#tno_filter_queue_20 <- filter(tno_sim_20_16_2, referral < target_20_date_2$dates)
...
Extra_tno_graph_20241112 <-
  ...
  #geom_point(data=tno_t4_date_20_2, shape=4, col = colours[2], size = 5, stroke = 2)+
  #geom_point(data=target_20_date_5, shape=4, col = colours[2], size = 5, stroke = 2)+
  ...
  # geom_ribbon(data= data.frame(dates= tno_queue16_2$dates
  #                              , ymin = tno_queue16_2$queue_size
  #                              , ymax = tno_20_adjusted_queue$queue_size)
  #             , aes(ymin=ymin, ymax=ymax, y=NULL), col=colours[2], fill=colours[2], alpha=0.4)+
  geom_line(col=colours[1], linetype="dotted") +
  geom_line(data=tno_queue16_2, col=colours[1]) +
  # geom_ribbon(data= data.frame(dates= tno_queue16_2$dates
  #                              , ymin = tno_queue16$queue_size
  #                              , ymax = tno_queue16_2$queue_size)
  #             , aes(ymin=ymin, ymax=ymax, y=NULL), col=colours[1], fill=colours[1], alpha=0.4)+
...
ggsave("./output/tno_20_black_2.png", scale = 0.8)
