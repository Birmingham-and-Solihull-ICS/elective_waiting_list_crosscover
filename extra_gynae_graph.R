...
#gynae_filter_queue_20 <- filter(gynae_sim_20_16_5, referral < target_20_date_5$dates)
...
# #### Adjusted plot
gynae_20_adjusted_queue_5 <- gynae_queue_20_16_5

gynae_20_adjusted_queue_2_5 <-
  gynae_queue_20_16_5 %>% 
  filter(meet_future_20 == 1)

gynae_20_adjusted_queue_2_5$queue_size <-rpois(nrow(gynae_20_adjusted_queue_2_5), target_queue_size_20_5)

gynae_20_adjusted_queue_5<-
  gynae_20_adjusted_queue_5 %>% 
  left_join(gynae_20_adjusted_queue_2_5, by = "dates", keep = TRUE) %>% 
...
Extra_gynae_graph <-
  ...
  geom_point(data=gynae_t4_date_40, shape=4, col = colours[2], size = 5, stroke = 2)+
  ...
  geom_line(col=colours[2], data=gynae_20_adjusted_queue) +
  geom_ribbon(data= data.frame(dates= gynae_40_adjusted_queue$dates
                               , ymin = gynae_40_adjusted_queue$queue_size
                               , ymax = gynae_20_adjusted_queue$queue_size)
              , aes(ymin=ymin, ymax=ymax, y=NULL), col=colours[2], fill=colours[2], alpha=0.4)+
  geom_line(col=colours[1]) +
  geom_line(data=gynae_queue16_2, col=colours[1]) +
  geom_ribbon(data= data.frame(dates= gynae_queue16_5$dates
                               , ymin = gynae_queue16_5$queue_size
                               , ymax = gynae_queue16_2$queue_size)
  ...
  geom_text(data = gynae_t4_date_40
            , label="T4"
            ,# y=8000
            , col="black"
            , size=4
...
#gynae_filter_queue_20 <- filter(gynae_sim_20_16_2, referral < target_20_date_2$dates)

gynae_20_adjusted_queue_2 <- gynae_queue_20_16_2

gynae_20_adjusted_queue_2_2 <-
  gynae_queue_20_16_2 %>% 
  filter(meet_future_20 == 1)

gynae_20_adjusted_queue_2_2$queue_size <-rpois(nrow(gynae_20_adjusted_queue_2_2), target_queue_size_20_2)

gynae_20_adjusted_queue_2<-
  gynae_20_adjusted_queue_2 %>% 
  left_join(gynae_20_adjusted_queue_2_2, by = "dates", keep = TRUE) %>% 
  mutate(dates = dates.x, queue_size = coalesce(queue_size.y, queue_size.x), meet_target = meet_target.x,
         meet_future_20 = meet_future_20.x) %>% 
  select(dates, queue_size, meet_target, meet_future_20)
...
Extra_gynae_graph_20241112 <-
  ...
  geom_vline(xintercept = programme_dts$startdate[2], alpha=1
             , colour="black")+
  geom_vline(xintercept = gynae_t4_date_20_2$dates, alpha=1
             , colour="black")+
  geom_vline(xintercept = target_20_date_2$dates, alpha=1
             , colour="black")+
  
  #geom_point(data=gynae_t4_date_20_2, shape=4, col = colours[2], size = 5, stroke = 2)+
  #geom_point(data=target_20_date_5, shape=4, col = colours[2], size = 5, stroke = 2)+
  ...
  # geom_line(col=colours[2], data=gynae_queue_20_16_5) +
  #geom_line(col=colours[2], data=gynae_40_adjusted_queue) +
  geom_line(col=colours[2], data=gynae_20_adjusted_queue_2) +
  # geom_ribbon(data= data.frame(dates= gynae_queue16_2$dates
  #                              , ymin = gynae_queue16_2$queue_size
  #                              , ymax = gynae_20_adjusted_queue$queue_size)
  #             , aes(ymin=ymin, ymax=ymax, y=NULL), col=colours[2], fill=colours[2], alpha=0.4)+
  geom_line(col=colours[1], linetype="dotted") +
  geom_line(data=gynae_queue16_2, col=colours[1]) +
  # geom_ribbon(data= data.frame(dates= gynae_queue16_2$dates
  #                              , ymin = gynae_queue16$queue_size
  #                              , ymax = gynae_queue16_2$queue_size)
  #             , aes(ymin=ymin, ymax=ymax, y=NULL), col=colours[1], fill=colours[1], alpha=0.4)+
 ...
 # geom_point(data=target_40_date_5, shape=4, col = colours[2], size = 5, stroke = 2)+
  ...
  geom_text(data = gynae_t4_date_20_2
            , label = "T4"
            #, y=8000
            ,, y=9200
            , col="black"
  ...
  geom_text(data = data.frame(dates =  as.Date(c("01-01-2025", "01-12-2024"), format = "%d-%m-%Y")
  ...
  labs(
    title = bquote(bold("Gynaecology: ") ~ "First GP referral to first Outpatients waiting list (2% relief capacity after Sept-24):"),
